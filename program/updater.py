...
async def update_repo(_, message: Message):
    ...
    msg = await message.reply("🔄 `برمجه وكتابة مصطفى ...`")
    update_avail = updater()
    if update_avail:
        await msg.edit("👤 يمكن شراء بوت بحقوقك كامل \n\n• او نسخه سورس بحقوقك بأرخص الأسعار حسابي الوحيد : @QQQLO.")
        system("git pull -f && pip3 install -r requirements.txt")
        execle(sys.executable, sys.executable, "main.py", environ)
        return
    await msg.edit("My only **account on ** telegram [سورس فريدوم •](https://t.me/QQQLO)", disable_web_page_preview=True)
